...
<script>
	$(function() {
		...
		document.getElementById("file").addEventListener("change", function () {
		    const fileName = this.files[0]?.name || "첨부파일"; // 선택된 파일 이름 가져오기
		    document.querySelector(".upload-name").value = fileName; // upload-name의 value 설정
		});
		
		 // 서버에서 전달된 내용을 JavaScript 변수로 전달
	    const brdContent = `${qna.brdContent}`;
	...
	function fn_submit() {
		...
		formData.append("contentHtml", contentHtml);
 		...
 		$.ajax({
			...
			url : "/course/courseQnAUpdateProc",
			data : formData,
			...
			<section class="board-container">
				<form class="board-form" id="writeForm" name="writeForm">
					<div class="form-group">
						...
						<input type="text" id="userId"  placeholder="${qna.userId }" readonly>
					</div>
					<div class="form-group">
						<label for="title_">이메일</label>
						<input type="text" id="userEmail"  placeholder="${qna.userEmail }" readonly>
					</div>
					<div class="form-group">
						<label for="title_">제목</label>
						<input type="text" id="title" name="title" value="${qna.brdTitle }" placeholder="제목을 입력해주세요.">
					</div>
					
	              <div class="filebox">
	              <c:choose>
	              	<c:when test="${!empty qna.courseListFile }">
	              		<input class="upload-name" value="${qna.courseListFile.fileOrgName }"  placeholder="첨부파일" readonly>
	              	</c:when>
	              	<c:otherwise>
	             		<input class="upload-name" value="첨부파일" placeholder="첨부파일" value="" readonly>
	              	</c:otherwise>
	              </c:choose>
...
<%-- 					<div class="form-group">
				...
				<input type="hidden" name="brdSeq" id="brdSeq" value="${brdSeq }">
   				<input type="hidden" name="courseCode" id="courseCode" value="1">	
