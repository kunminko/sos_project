...
<script>
	$(function() {
	    ...
	    const brdContent = `${review.brdContent}`;
		...
		document.querySelectorAll('.star-rating .star').forEach(star => {
	        ...
	        const ratingValue = parseInt(document.getElementById('ratingValue').value, 10); // 기존 별점 값
	...
	function fn_submit() {
		...
		formData.append("ratingValue", ratingValue);
 		...
 		$.ajax({
			...
			url : "/course/courseReviewUpdateProc",
			data : formData,
			...
			<section class="board-container">
				<form class="board-form" id="writeForm" name="writeForm">
					...
					<div class="form-group">
						...
						<input type="text" id="userEmail" value="${review.userEmail }" readonly>
					</div>
					<div class="form-group">
						<label for="title_">제목</label>
						<input type="text" id="title" name="title" value="${review.brdTitle }" placeholder="제목을 입력해주세요.">
					</div>
...
<!-- 	              <div class="filebox">
				...
				<div class="form-group">
				    ...
				    <input type="hidden" id="ratingValue" name="rating" value="${review.brdRating }"> <!-- 선택한 별점 값 -->
				</div>
